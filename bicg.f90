...
!		IF ((restart .eq. 0) .or. (restart .eq. n)) THEN
!			restarted = .false.
!		ELSE
!			restarted = .true.
!		ENDIF
...
!		IF (restarted) THEN
!			outer = maxit
!			IF (restart .gt. n) THEN
!				WRITE (stdout, '(a)') 'WARNING : GMRES requires too many inner restarts '
!				restart = n
!			ENDIF
!			inner = restart
!		ELSE
!			outer = 1
!			IF (restart .gt. n) THEN
!				WRITE (stdout, '(a)') 'WARNING : GMRES requires too maxit '
!				maxit = n
!			ENDIF
!			inner = maxit
!		ENDIF
...
!!		Preallocate J to hold the Given's rotation constraints
!		ALLOCATE(J(2,inner))
!		ALLOCATE(my_U(my_nrows,inner))
!		ALLOCATE(R(inner,inner))
!		ALLOCATE(w(inner+1))
		
!		DO outeriter = 1, outer
!			!  Construct u for Householder reflector.
!			!  u = r + sign(r(1))*||r||*e1
!			my_u = my_r
!			IF(root_node) THEN
!				beta = scalarsign(my_r(1))*normr
!				my_u(1) = my_u(1) + beta
!			ENDIF
!			my_U(:,1) = my_u
...
!				! A*v
!				CALL matvectcoo(my_Amat, my_RowInd, my_ColInd, my_nnz, &
!								my_v, my_v, my_nrows, everyones_rows, 'N')

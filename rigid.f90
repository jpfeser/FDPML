...
subroutine rgd_blk (nr1,nr2,nr3,nat,dyn,q,tau,epsil,zeu,bg,omega,sign)
  ...
  complex(RP) :: dyn(3,3,nat,nat) ! dynamical matrix
  real(RP) &
       q(3),           &! q-vector
  ...
  ! local variables
  !
  real(RP):: geg                    !  <q+G| epsil | q+G>
  integer :: na,nb, i,j, m1, m2, m3
  integer :: nr1x, nr2x, nr3x
  real(RP) :: alph, fac,g1,g2,g3, facgd, arg, gmax
  real(RP) :: zag(3),zbg(3),zcg(3), fnat(3)
  complex(RP) :: facg
  !
  ...
  !
  IF (abs(sign) /= 1.0_RP) THEN
       WRITE(*,*) ' ERROR : wrong value for sign'
       STOP
  ENDIF
  !
  ...
  do m3 = -nr3x,nr3x
     ...
     !
     if (geg > 0.0_RP .and. geg/alph/4.0_RP < gmax ) then
        !
        ...
        do nb = 1,nat
           ...
           do na = 1,nat
              ...
              arg = 2.d0*pi* (g1 * (tau(1,na)-tau(1,nb))+             &
                              ...
                              g3 * (tau(3,na)-tau(3,nb)))
              !
              facg = facgd * CMPLX(cos(arg),sin(arg),kind=RP)
              do j=1,3
                 do i=1,3
...
subroutine nonanal(nat, nat_blk, itau_blk, epsil, q, zeu, omega, dyn )
 ...
 !
 complex(RP), intent(inout) :: dyn(3,3,nat,nat) ! dynamical matrix
 real(RP), intent(in) :: q(3),  &! polarization vector
      &       epsil(3,3),     &! dielectric constant tensor
 ...
 ! local variables
 !
 real(RP) zag(3),zbg(3),  &! eff. charges  times g-vector
      &       qeq              !  <q| epsil | q>
...
!print*, q(1), q(2), q(3)
 if (qeq < 1.d-8) then
    write(*,'(5x,"A direction for q was not specified:", &
...
!-----------------------------------------------------------------------
subroutine nonanal_ifc(nat, nat_blk, itau_blk, epsil, q, zeu, omega, nr1,nr2,nr3,f_of_q )
  !-----------------------------------------------------------------------
  !     add the nonanalytical term with macroscopic electric fields
  !
  use kinds, only: RP
 ...
 !                atom na in the supercell
 !
 real(RP), intent(out) :: f_of_q(3,3,nat,nat) ! dynamical matrix
 real(RP), intent(in) :: q(3),  &! polarization vector
      &       epsil(3,3),     &! dielectric constant tensor
      &       zeu(3,3,nat_blk),   &! effective charges tensor
      &       omega            ! unit cell volume
...
subroutine dyndiag (nat,ntyp,amass,ityp,dyn,w2,z)
  ...
  complex(RP) dyn(3,3,nat,nat)
  real(RP) amass(ntyp)
  ! output
  real(RP) w2(3*nat)
  complex(RP) z(3*nat,3*nat)
  ! local
  real(RP) diff, dif1, difrel
  integer nat3, na, nta, ntb, nb, ipol, jpol, i, j
  complex(RP), allocatable :: dyn2(:,:)
  real(RP) :: q(3)
  !
 ...
 ! if(io_node) call write_dyn_on_file(q,dyn,nat)
  ...
  difrel=0.d0
  do i = 1,nat3
     dyn2(i,i) = CMPLX( DBLE(dyn2(i,i)),0.d0,kind=RP)
     do j = 1,i - 1
  ...
  if ( diff > 1.d-6 ) write (*,'(5x,"Max |d(i,j)-d*(j,i)| = ",f9.6,/,5x, &
...
subroutine cdiagh2 (n,h,ldh,e,v)
  ...
  complex(KIND = RP) ::  h(ldh,n)  ! matrix to be diagonalized
  !
  ! on OUTPUT
  real(KIND = RP)  ::   e(n)      ! eigenvalues
  complex(RP) :: v(ldh,n)  ! eigenvectors (column-wise)
  !
...
!       &           ILAENV,  &! function which gives block size
  ...
  complex(KIND = RP), allocatable:: work(:)
  !

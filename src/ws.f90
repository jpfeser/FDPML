...
!
	...
	real(RP) rws(0:3,nrwsx), atw(3,3)
	REAL(KIND = RP), PARAMETER :: eps=1.0d-6
	...
	do ir=-nx,nx
		do jr=-nx,nx
			do kr=-nx,nx
				...
				if (ii.gt.nrwsx) then
					write(*,*) ' ERROR '
					write(*,*) ' wsinit : ERROR in generating the weights'
...
!
	...
	real(KIND = RP) ::  r(3), rrt, ck, rws(0:3,nrws), wsweight
	REAL(KIND = RP), PARAMETER:: eps=1.0d-6
!
	wsweight = 0.d0
	nreq = 1
